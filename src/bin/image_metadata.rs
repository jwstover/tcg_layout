...
use std::env;
use std::path::Path;
use tcg_layout::image::{load_image_metadata, detect_card_type_by_aspect_ratio, extract_dpi_from_metadata};
...
fn main() -> Result<()> {
    ...
    match load_image_metadata(image_path) {
        Ok(metadata) => {
            ...
            println!("  Dimensions: {}x{} pixels", metadata.dimensions.0, metadata.dimensions.1);
            ...
            match metadata_dpi {
                ...
                None => {
                    ...
                    if let Some(card_type) = detect_card_type_by_aspect_ratio(metadata.dimensions) {
                        ...
                        println!("  Image aspect ratio: {:.3}", metadata.dimensions.0 as f32 / metadata.dimensions.1 as f32);
