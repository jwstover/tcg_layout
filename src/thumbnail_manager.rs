...
impl ThumbnailManager {
    ...
    pub fn request_thumbnail(&mut self, path: PathBuf) -> Option<ImageBuffer<image::Rgba<u8>, Vec<u8>>> {
        ...
        let request = ThumbnailRequest::Load { path: path.clone(), response_tx };
    ...
    pub fn try_recv_message(&mut self) -> Option<ThumbnailMessage> {
        match self.message_rx.try_recv() {
            Ok(message) => {
                ...
                let ThumbnailMessage::ThumbnailLoaded { ref path, ref result } = message;
    ...
    async fn test_thumbnail_manager_success() {
        ...
        for _ in 0..10 {
            if let Some(message) = manager.try_recv_message() {
                match message {
                    ThumbnailMessage::ThumbnailLoaded { path: msg_path, result } => {
        ...
        assert!(received_message, "Should have received thumbnail loaded message");
    }
